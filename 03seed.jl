...
draw_step!(ax, step::Symbol; radius::Real, center::Point2f, linewidth::Real, color, rotation::Real, method::Symbol, pointsize::Real) = begin
    if step === :point
...
seed_step_figure(step::Symbol, radius::Real; center::Point2f=Point2f(0,0), linewidth::Real=2.5, color=colorant"rgba(0, 0, 0, 0.8)", background=:white, rotation::Real=0, pad_ratio::Real=0.08, method::Symbol=:arc, pointsize::Real=radius*0.06) = begin
    res = (Int(round(4*radius)), Int(round(4*radius)))
    f = Figure(size=res, backgroundcolor=background)
    ax = Axis(f[1, 1]; aspect=DataAspect(), limits=seed_limits(radius, center; pad_ratio=pad_ratio))
    hidedecorations!(ax); hidespines!(ax)
    draw_step!(ax, step; radius=radius, center=center, linewidth=linewidth, color=color, rotation=rotation, method=method, pointsize=pointsize)
    (f, ax)
end

seed_strip_figure(radius::Real; steps=(:point, :circle, :seed), center::Point2f=Point2f(0,0), linewidth::Real=2.5, color=colorant"rgba(0, 0, 0, 0.8)", background=:white, rotation::Real=0, pad_ratio::Real=0.08, method::Symbol=:arc, pointsize::Real=radius*0.06, gap::Float64=6.0) = begin
    n = length(steps)
    ...
    for (i, s) in enumerate(steps)
        ...
        hidedecorations!(ax); hidespines!(ax)
        draw_step!(ax, s; radius=radius, center=center, linewidth=linewidth, color=color, rotation=rotation, method=method, pointsize=pointsize)
    end
    f
end

seed_strip_svg(path::AbstractString, radius::Real; steps=(:point, :circle, :seed), center::Point2f=Point2f(0,0), linewidth::Real=2.5, color=colorant"rgba(0, 0, 0, 0.8)", background=:white, rotation::Real=0, pad_ratio::Real=0.08, method::Symbol=:arc, pointsize::Real=radius*0.06) = begin
    f = seed_strip_figure(radius; steps=steps, center=center, linewidth=linewidth, color=color, background=background, rotation=rotation, pad_ratio=pad_ratio, method=method, pointsize=pointsize)
    save(path, f)
    path
end
